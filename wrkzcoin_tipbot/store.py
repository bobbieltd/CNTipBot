...
scanCount = 0 # rescan whole blockchain once per 1000 scans for certainty
...
async def sql_add_donation(userID: str, change_amount: int, coin: str = None) -> str:
    # TODO
    global conn
    if coin in ENABLE_COIN:
        try:
...
async def sql_add_gaming(userID: str, change_amount: int, coin: str = None) -> str:
    global conn
    COIN_NAME = coin.upper()
    if coin in ENABLE_COIN:
        try:
